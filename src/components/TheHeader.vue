<script setup>

</script>

<template>
  <div class="container-auto header">
    <a href="/"
      ><h1 class="header__logo">DETAILING <br />GURU</h1></a
    >
    <nav class="header__menu menu">
      <ul class="header__list menu__body">
        <li class="menu__item">
          <a class="header__link active" href="/">Главная</a>
        </li>
        <li>
          <a class="header__link" href="/favorites">Избранное</a>
        </li>
        <li>
          <a class="header__link" href="/about">О нас</a>
        </li>
      </ul>
      <div class="header-account">
        <div class="header-btns">
          <a href="/login" class="header__btn login">Войдите/</a>
          <a class="header__btn register" href="/register">Зарегистрируйтесь</a>
        </div>
        <button class="header__icon">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g clip-path="url(#clip0_450_239)">
              <path
                d="M10 10C10.9889 10 11.9556 9.70676 12.7779 9.15735C13.6001 8.60794 14.241 7.82705 14.6194 6.91342C14.9978 5.99979 15.0969 4.99446 14.9039 4.02455C14.711 3.05465 14.2348 2.16373 13.5355 1.46447C12.8363 0.765206 11.9454 0.289002 10.9755 0.0960758C10.0055 -0.0968503 9.00021 0.00216643 8.08658 0.380605C7.17295 0.759043 6.39206 1.39991 5.84265 2.22215C5.29324 3.0444 5 4.0111 5 5C5.00132 6.32568 5.52853 7.59668 6.46593 8.53407C7.40332 9.47147 8.67432 9.99868 10 10ZM10 1.66667C10.6593 1.66667 11.3037 1.86217 11.8519 2.22844C12.4001 2.59471 12.8273 3.1153 13.0796 3.72439C13.3319 4.33348 13.3979 5.0037 13.2693 5.6503C13.1407 6.29691 12.8232 6.89085 12.357 7.35703C11.8908 7.8232 11.2969 8.14067 10.6503 8.26929C10.0037 8.3979 9.33348 8.33189 8.72439 8.0796C8.1153 7.82731 7.59471 7.40007 7.22843 6.8519C6.86216 6.30374 6.66667 5.65927 6.66667 5C6.66667 4.11595 7.01786 3.2681 7.64298 2.64298C8.2681 2.01786 9.11594 1.66667 10 1.66667Z"
                fill="#454545"
              />
              <path
                d="M10 11.6667C8.01155 11.669 6.10518 12.4598 4.69914 13.8659C3.29309 15.2719 2.50221 17.1783 2.5 19.1667C2.5 19.3878 2.5878 19.5997 2.74408 19.756C2.90036 19.9123 3.11232 20.0001 3.33333 20.0001C3.55435 20.0001 3.76631 19.9123 3.92259 19.756C4.07887 19.5997 4.16667 19.3878 4.16667 19.1667C4.16667 17.6197 4.78125 16.1359 5.87521 15.042C6.96917 13.948 8.4529 13.3334 10 13.3334C11.5471 13.3334 13.0308 13.948 14.1248 15.042C15.2188 16.1359 15.8333 17.6197 15.8333 19.1667C15.8333 19.3878 15.9211 19.5997 16.0774 19.756C16.2337 19.9123 16.4457 20.0001 16.6667 20.0001C16.8877 20.0001 17.0996 19.9123 17.2559 19.756C17.4122 19.5997 17.5 19.3878 17.5 19.1667C17.4978 17.1783 16.7069 15.2719 15.3009 13.8659C13.8948 12.4598 11.9884 11.669 10 11.6667Z"
                fill="#454545"
              />
            </g>
            <defs>
              <clipPath id="clip0_450_239">
                <rect width="20" height="20" fill="white" />
              </clipPath>
            </defs>
          </svg>
        </button>
      </div>
    </nav>
    <main>
      <RouterView />
    </main>
    <div class="menu-icon"><span></span><span></span><span></span></div>
  </div>
</template>

<style scoped>
.header__logo {
  font-family: BOWLER;
  font-size: 32px;
  color: #000;
}

.header {
  padding-top: 40px;
  display: flex;
  align-items: center;
  justify-content: space-between;
}

.header__list {
  display: flex;
  align-items: center;
  gap: 45px;
  flex-grow: 1;
  justify-content: center;
}

.header__menu {
  display: flex;
  align-items: center;
  flex: 1;
}

.header__link {
  font-size: 16px;
  color: #454545;
  font-weight: 600;
  padding: 12px 27px;
  transition: color 0.4s ease-in-out;
}

.header__link:hover {
  color: #10141a;
}

.active {
  color: #fff;
  border-radius: 73px;
  background: #10141a;
}

.active:hover {
  color: #fff;
}

.header-account {
  display: flex;
  align-items: center;
  gap: 35px;
}

.header-btns {
  display: flex;
  flex-direction: column;
}

.header__btn {
  color: #454545;
  font-size: 16px;
  font-weight: 600;
  transition: color 0.4s ease-in-out;
}

.header__btn:hover {
  color: #10141a;
}

.header__icon {
  width: 50px;
  height: 50px;
  background-color: #fff;
  border-radius: 50%;
  padding: 10px;
  transition: all 0.4s ease-in-out;
}

.header__icon:hover {
  background-color: #d4d4d4;
}

.menu-icon {
  display: none;
}
</style>
